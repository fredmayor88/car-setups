...
5. **Map keys to the catalog, then write — append only.** Load the car's Notion `Parameters`
   ...
   to the user rather than dropping it. **Validate each value against the catalog row** — it
   should fall within `Min..Max`, or be a member of `Discrete steps` when that column is filled;
   **flag mismatches** rather than dropping them (a save value outside the catalog usually means
   the catalog is incomplete, not that the value is wrong). Then add one new
