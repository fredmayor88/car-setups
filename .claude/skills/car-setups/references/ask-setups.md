...
- **The question** — free-form. May be conceptual, or about a named setup, or a comparison of
  two setups.
- **Optional anchors** — a setup name (e.g. `alsace gpt1`), a car, a stage, a surface, a
...
- **User guidelines** — if the question is about *how to tune* and the user has a global
  ...
  overrides the base (later wins: base → global guidelines → surface section → per-car → stage).
  Only read it if it's readily reachable within `Car setups → {Game}`; don't go hunting.
...
Apply only lines tagged `[All]` **or the car's drivetrain**.

### 3d. Load stage context
Fetch the `{stage}` page under the car's setups section (surface description, key
corners/speeds, what the driver was aiming for). This is the most specific layer and takes
precedence on any point it speaks to directly.

### 3e. Explain the value
For each parameter the user asked about (or, if they asked "explain this setup" broadly, the
handful that matter most for the stage):
...
- Give the **reason it's there**: cite the layer that drives it — a base principle, a user
  guideline, the surface bias, or the stage description. When a user guideline or the stage
  description is the reason, **say so explicitly** ("because your guidelines say you like a planted
  rear", "because this stage is described as fast and bumpy").
...
- Tie it back to **why** each setup went that way: each setup's `Surface`, `Stage` description,
  and intent (a tarmac stage wants stiffer/lower; a bumpy gravel stage wants softer/higher). If
  the setups are for different surfaces, lead with that — it explains most differences.
...
- **Layered — the user wins.** When a user guideline or stage description drives the answer, cite
  it; their preference overrides the base.
- **Grounded and concrete** — explain in terms of real levers and effects; never invent a
  ...
  support (if you can't tell why, say it's within range and plausibly a stage/taste choice).
